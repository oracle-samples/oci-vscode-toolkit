 ...
 */
 ...
 import { BaseNode } from './tree/nodes/base-node';
 ...
 export interface IOCIApi {
     ...
     // default root folder path to handle all user created resources (files, directories etc.,)
     getDefaultArtifactFolderPath: (subFolderPath: string) => string;

     // facade to manage the life-cycle of all user created resources (files, directories etc.,)
...
export interface IFileExplorer{
	GetChildNodes(path: FileExplorerItem) : Promise<IRootNode[]>;
}
...
export class SandboxFolder {     
    ...
    relativePathFromAbsolute(absolutePath: string) : string;    
} 
